                ...
                if($this->config['sync_direction']==1){ //Не используем импорт товаров и категорий из 1c при режиме 0
                    ...
                    switch ($step){
                        case 'importCategories':
                            ...
                            //$out = 'progress'.PHP_EOL.'Категории выгружены в временную таблицу'.PHP_EOL;;
                            break;

                        case 'prepareCategories':
                           $out = $this->prepareCategories();
                            // $out = 'Пропускаем подготовку категорий'.PHP_EOL;
                            break;
	...
	private function importCategories($filename){
		...
		if($reader->name == 'Группы'){
		    try {
                ...
                copy($this->config['temp_dir'].$filename, $this->config['temp_dir'].'/../export/'.$filename);
		    }
			$reader->close();
         ...
         */
        $this->options['totalCategories'] = $_SESSION['totalCategories'] = 0;
	...
	private function importProducts($filename){
		...
		while($reader->name === 'Товар'){
			if($firstImport || $currentImportProduct > $lastImportProduct){
                ...
                //custom properties
                $prod_properties['Производитель']= addslashes((string) $xml->Производитель);
                $prod_properties['ПолноеНаименование']= addslashes((string) $xml->ПолноеНаименование);
                $prod_properties['ОсновнаяКатегория']= addslashes((string) $xml->ОсновнаяКатегория);
                $prod_properties['Подкатегория']= addslashes((string) $xml->Подкатегория);
                $prod_properties['Штрихкод']= addslashes((string) $xml->Штрихкод);
                $prod_properties['Ингредиенты']= addslashes((string) $xml->Ингредиенты);
                $prod_properties['КоличествоПорций']= addslashes((string) $xml->КоличествоПорций);
                $prod_properties['ВесПозиции']= addslashes((string) $xml->ВесПозиции);
                $prod_properties['Упаковка']= addslashes((string) $xml->Упаковка);
                $prod_properties['ПоставленнаяЦель']= addslashes((string) $xml->ПоставленнаяЦель);
                $prod_properties['Наличие']= addslashes((string) $xml->Наличие);
                $prod_properties['ШейкерВПодарок']= addslashes((string) $xml->ШейкерВПодарок);
                $prod_properties['БесплатнаяДоставка']= addslashes((string) $xml->БесплатнаяДоставка);
                $prod_properties['ПодЗаказ']= addslashes((string) $xml->ПодЗаказ);
                $prod_properties['Популярный']= addslashes((string) $xml->Популярный);
                $prod_properties['ЦенаВВалюте']= addslashes((string) $xml->ЦенаВВалюте);
                $prod_properties['ЦенаСоСкидкой']= addslashes((string) $xml->ЦенаСоСкидкой);
                $prod_properties['Валюта']= addslashes((string) $xml->Валюта);

                //Вкус

                if(isset($xml->ХарактеристикиТовара)){
                    $prod_properties['СписокВкусов'] = $xml->ХарактеристикиТовара->ХарактеристикаТовара->Значение;
                    $prod_properties['ВкусыКоличество'] = $xml->ХарактеристикиТовара->ХарактеристикаТовара->Количество;
                }
