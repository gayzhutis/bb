...
class sxlsx {
     ...
     public function generate($name = '') {
          ...
          foreach($this->_tpls as $file => &$content) {
               $content = file_get_contents(dirname(__FILE__) . self::DS . 'templates' . $file);
          ...
          if(is_writable($this->_sxlsxPath) || is_writable(dirname($this->_sxlsxPath))) {
               include_once(dirname(__FILE__) . sxlsx::DS . 'zipfile.php');
               ...
               file_put_contents($this->_sxlsxPath, $zip->file());
               // chmod($this->_sxlsxPath, 0777);
          } else {
               throw new sxlsx_Exception('could not write to file "' . $dist . '"');
          }
