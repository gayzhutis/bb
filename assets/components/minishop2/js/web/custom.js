...
(function(window, document, $, undefined) {
	...
	miniShop2.Cart = {
		...
		,status: function(status) {
			if (status['total_count'] < 1) {
				...
				/*------правка дефолтного js-------*/
				$.ajax({//обновление корзины по аяксу
                    type: "POST",
                    async:false,
                    ...
                    success: function(msg){
                        $('.mc-prod-container').empty().append(msg);
                    }
                });
				
				if ($(".goods-list-content").height() > 340) $(".goods-list").css("overflow-y","auto");//добавление автопрокрутки в миникорзине
                /*Дописываем окончание в слове товар(-а/-ов)*/
                var lastSymbols = status.total_count%10;
                var lastTwoSymbols = status.total_count%100;
		...
		,remove_position: function(key) {
		    $('#'+key+'.table-row').remove();//правкa дефолтного js
			$('#' + key).remove();
		}
	};
	...
	miniShop2.Gallery = {
		...
		,initialize: function() {
            ...
            $( miniShop2.Gallery.gallery + ' ' + miniShop2.Gallery.thumbnail).first().parent().addClass("current");
            //var url = $('#product-image-content').data('image');
            //$('#product-image-content .zoomImg').remove();
            //$('#product-image-content').zoom({url: url});
            
			if ($(miniShop2.Gallery.gallery).length) {
				miniShop2.$doc.on('hover', miniShop2.Gallery.gallery + ' ' + miniShop2.Gallery.thumbnail, function(e) {
					var url = $(this).attr('href'),
                        src = $(this).data('image');
                        
					$(miniShop2.Gallery.gallery + ' ' + miniShop2.Gallery.thumbnail).parent().removeClass("current");
					$(this).parent().addClass("current");
					
					$(miniShop2.Gallery.mainImage, miniShop2.Gallery.gallery).attr('src', src).parent().data('image', url);
					e.preventDefault();
				});
	...
	miniShop2.Order = {
		...
		,getcost: function() {
			...
			callbacks.getcost.response.success = function(response) {
			    $('.ms2_total_cost').text(miniShop2.Utils.formatPrice(response.data['cost']));
		...
		,submit: function() {
			...
			callbacks.submit.before = function() {
			    $('.preload-overlay, .products-preload').css('display','block');/*--pravka---*/
			...
			callbacks.submit.response.error = function(response) {
			    $('.preload-overlay, .products-preload').css('display','none');/*---pravka---*/
