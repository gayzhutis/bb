var mSearch2 = {
	...
	,handleSort: function() {
		...
		$(document).on('click', this.options.sort_link, function(e) {
            ...
            $(".sorting-content").removeClass("visible");
	...
	,handleLimit: function() {
		$(document).on('click', this.options.limit, function(e) {
			...
			$(mSearch2.options.limit).removeClass("current");
	...
	,beforeLoad: function() {
		...
		this.filters.find("input:not(:checked)").parent().removeClass(this.options.checked_class);
		this.filters.find("input:checked").parent().addClass(this.options.checked_class);
	}
